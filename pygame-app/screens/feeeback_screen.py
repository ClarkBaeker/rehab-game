...
class FeedbackScreen:
    def __init__(self, manager):
        ...
        self.feedback_rect = {
            "happy": pygame.Rect(manager.screen_width//2 - feedback_boxes_size // 2 + feedback_boxes_distance, y_pos, feedback_boxes_size, feedback_boxes_size),
            "medium":   pygame.Rect(manager.screen_width//2 - feedback_boxes_size // 2, y_pos, feedback_boxes_size, feedback_boxes_size),
            "sad":   pygame.Rect(manager.screen_width//2 - feedback_boxes_size // 2 - feedback_boxes_distance, y_pos, feedback_boxes_size, feedback_boxes_size)
        }
        self.feedback_buttons = {
            "happy": InvisibleButton(manager, rect=self.feedback_rect["happy"], callback=lambda: self.use_feedback("happy")),
            "medium": InvisibleButton(manager, rect=self.feedback_rect["medium"], callback=lambda: self.use_feedback("medium")),
            "sad": InvisibleButton(manager, rect=self.feedback_rect["sad"], callback=lambda: self.use_feedback("sad")),
        }
