...
def interpolate_bad_angles(angle, lower_bound=-50, upper_bound=50):
    ...
    angle[~valid_mask] = np.interp(indices[~valid_mask],
                                indices[valid_mask],
                                angle[valid_mask])
    return angle
...
def animate_kinetics_knee(timepoints_in_ms_angle, angle,
                        timepoints_in_s, finger_x, finger_y):
    ...
    factor = len(timepoints_in_s) / n_frames

    # For real-time playback, interval in ms
    interval_ms = int(dt_angle * 1000)

    # Norm across all time-based color lines to keep the colors consistent
    max_time_val = max(timepoints_in_s[-1], timepoints_in_s_angle[-1])
    ...
    def update(frame):
        ...
        # Partial index in the trajectory arrays
        idx_traj = int(frame * factor)

        # Partial data for the trajectory
        t_traj_part = timepoints_in_s[:idx_traj]
        x_part = finger_x[:idx_traj]
        y_part = finger_y[:idx_traj]

        # Create a new partial line collection for the trajectory
        if len(x_part) > 1:
            traj_lc = colorline_2d(ax_traj, t_traj_part, x_part, y_part, norm, cmap="cividis")
        ...
        for i, ax in enumerate(axs):
            ...
            ax.set_ylim(np_angle_arr_min[i], np_angle_arr_max[i])
    ...
    plt.tight_layout()
    plt.show()
    # ani.save("kinetics_anim.mp4", fps=1/dt_angle)
...
def load_data(csv_file, has_headers=False):
    ...
    df['time_in_microseconds'] = pd.to_numeric(df['time_in_microseconds'], errors='coerce')
    df['timepoint'] = df['timepoint'].astype(str)

    # Drop invalid rows
    df.dropna(subset=['timepoint', 'time_in_microseconds', 'finger_x', 'finger_y'], inplace=True)

    # Convert time strings to seconds
    timepoints = df['timepoint'].tolist()
    times_in_seconds = np.array([convert_time_to_seconds(tp) for tp in timepoints])
    times_in_seconds -= times_in_seconds[0]  # zero-based
    ...
    return timepoints, timepoints_in_ms, finger_x, finger_y
...
def load_angle_data(csv_file):
    ...
    if not {'timepoint','time_in_microseconds','knee_angle'}.issubset(df.columns):
        raise ValueError("CSV must have columns: timepoint, time_in_microseconds, knee_angle")
    ...
    timepoints = df['timepoint'].tolist()
    times_in_seconds = np.array([convert_time_to_seconds(tp) for tp in timepoints])
    times_in_seconds -= times_in_seconds[0]

    additional_time_in_ms = df['time_in_microseconds'].to_numpy() / 1000
    timepoints_in_ms = times_in_seconds * 1000 + additional_time_in_ms

    return timepoints, timepoints_in_ms, angle.to_numpy()


if __name__ == "__main__":

    traj_path = Path("C:/Users/adria/OneDrive/Dokumente/05_NISE/rehab-game/pygame-app/logs/clark_2/trajectory_20250204-124808.csv")
    angle_path = Path("C:/Users/adria/OneDrive/Dokumente/05_NISE/rehab-game/pygame-app/logs/clark_2/knee_angle_20250204-124808.csv")

    # Load 2D trajectory
    timepoints, timepoints_in_ms, finger_x, finger_y = load_data(traj_path, has_headers=True)
    timepoints_in_s = timepoints_in_ms / 1000.0
    ...
    # Load knee-angle
    timepoints_angle, timepoints_in_ms_angle, angle = load_angle_data(angle_path)
    min_angle = angle.min()
    max_angle = angle.max()
    timepoints_in_s_angle = timepoints_in_ms_angle / 1000.0
    ...
    # Smooth the angle
    dt_angle = np.mean(np.diff(timepoints_in_s_angle)) 
    angle_smooth = savgol_filter(angle, 15, 3) 

    # Number frames in the angle data
    n_frames = len(timepoints_in_s_angle)

    # To get real-time intervals => use average dt in seconds -> ms
    dt = dt_angle
    interval_ms = int(dt * 1000)

    # Map angle frames to trajectory frames, in case they're different lengths
    factor = len(timepoints_in_s) / n_frames

    # Animate the data
    animate_kinetics_knee(
        timepoints_in_ms_angle, angle,
        timepoints_in_s, finger_x, finger_y
    )
